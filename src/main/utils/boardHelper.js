...
export const prepareBlocks = (width, height, numberOfRows = DEFAULT_NUMBER_OF_ROWS) => {
    ...
    const blockHeight = Math.round(BLOCK_HEIGHT_RATIO * height);
    const blocks = [];
    for (let i = 0; i < numberOfRows; i += 1) {
        const rowYpos = yOffset + i * (blockHeight + BLOCK_SPACING);
        blocks.push(...prepareBlockRow(width, rowYpos, blockHeight, '#AAAAAA'));
    }
    return blocks;
};

const prepareBlockRow = (width, rowYpos, blockHeight, color) => {
